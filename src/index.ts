...
const getDataById = async (id: number) => new Promise<DataType>(
...
const getVocabById = async (id: number) => new Promise((resolve) => {
  database.all(`
    select * from vocabulary join vocabulary_info as info
    on info.vocabulary_id = vocabulary.id
    where vocabulary.id = ${id}
...
const updateVocabInfo = (
  ...
  database.exec(sql,
    async (err) => err ? console.log(err) : resolve((await getDataById(id)))
  );
});
...
ipcMain.on('search', async (event, arg) => {
  ...
  const id = arg?.id;
  const data = (id ? await getVocabById(id) : await search(keyword)) as DataType[];
  const results = [...data];
  await Promise.all(data.map(async (x) => {
    if (x.past) results.push({ ...(await getDataById(x.past)), hidden: true });
    if (x.future) results.push({ ...(await getDataById(x.future)), hidden: true });
    if (x.imperative) results.push({ ...(await getDataById(x.imperative)), hidden: true });
    if (x.link) results.push({ ...(await getDataById(x.link)), hidden: true });
  }));
...
ipcMain.on('create', async (event, arg) => {
  ...
  try {
    ...
    const result = [data];
    if (data.past) result.push({ ...(await getDataById(data.past)), hidden: true });
    if (data.future) result.push({ ...(await getDataById(data.future)), hidden: true });
    if (data.imperative) result.push({ ...(await getDataById(data.imperative)), hidden: true });
    if (data.link) result.push({ ...(await getDataById(data.link)), hidden: true });
...
ipcMain.on('edit', async (event, arg: { data: EditVocabDto; }) => {
  ...
  await updateVocab(vocabulary_id, page);
  await updateVocabInfo(id, rest);
  const data = await getDataById(vocabulary_id);
